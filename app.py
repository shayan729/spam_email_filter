...
# --- FIX: Consolidated Model & NLTK/spaCy Data Loading ---
try:
    ...
    print("✅ NLTK data (stopwords, wordnet) found.")

except (FileNotFoundError, IOError, LookupError) as e:
    # If any critical resource is missing and cannot be downloaded, stop the app.
    print(f"❌ Critical startup error: {e}. One or more essential models/data files are missing.")
    # Attempt to download NLTK data if it's the cause
    if isinstance(e, LookupError):
        print("⚠️ Attempting to download missing NLTK data...")
        nltk.download('stopwords')
        nltk.download('wordnet')
        print("✅ NLTK data downloaded. Please restart the application.")
...
# --- (All other functions from your previous version remain unchanged) ---
...
def analyze_headers(text):
    ...
    return {'is_spam': False, 'reason': "Subject line appears clean.", 'score': f"{len(found_keywords)}/{len(suspicious_keywords)} flags"}
...
def analyze_content_nlp(text):
    ...
    if urgency_score >= 3: return {'is_spam': True, 'reason': f"High urgency score ({urgency_score}) indicates pressure tactics."}
    doc = nlp(text)
...
def perform_analysis(email_text, links, attachments):
    ...
    for stage in pipeline:
        result = stage['fn'](email_text, links, attachments)
...
def analyze_text_route():
    ...
    res = perform_analysis(text, extracted_data['links'], extracted_data['attachments'])
...
def analyze_file_route():
    ...
    try:
        ...
        res = perform_analysis(parsed_data['full_text'], parsed_data['links'], parsed_data['attachments'])
        res.update(parsed_data)
        return jsonify(res)
